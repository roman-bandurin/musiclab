/**
 * Адаптер Better Auth → Nitro (те же эндпоинты и форматы: /api/login, /api/register, /api/logout).
...
export function createNitroAuthAdapter () {
  return {
    auth: {
    ...
    request: {
      'GET /api/auth/get-session': mapper(rules([[import.meta.env.VITE_API_URL, 'context.baseURL'], ['/api/auth/get-session', 'context.url']])),
